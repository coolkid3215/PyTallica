...
def Open(MidiNotes,notePos,strings):
    ...
    for i in range(len(MidiNotes)):
        if MidiNotes[i][0] <=71 and MidiNotes[i][0] >= 66:   #E4
            StringE4.append([i,MidiNotes[i][0],'StringE4'])
            MidiNotes[i][0] = 64
        elif MidiNotes[i][0] <= 65 and MidiNotes[i][0] >= 60:    #B3
            StringB3.append([i,MidiNotes[i][0],'StringB3'])
            MidiNotes[i][0] = 59
        elif MidiNotes[i][0] <= 59 and MidiNotes[i][0] >= 55: #G3
            StringG3.append([i,MidiNotes[i][0],'StringG3'])
            MidiNotes[i][0] = 55
        elif MidiNotes[i][0] <= 54 and MidiNotes[i][0] >= 50:    #D2
            StringD2.append([i,MidiNotes[i][0],'StringD2'])
            MidiNotes[i][0] = 50
        elif MidiNotes[i][0] <= 49 and MidiNotes[i][0] >=45: #A2
            StringA2.append([i,MidiNotes[i][0],'StringA2'])
            MidiNotes[i][0] =45
        elif MidiNotes[i][0] <=44 and MidiNotes[i][0] >=40:  #E2
            StringE2.append([i,MidiNotes[i][0],'StringE2'])
            MidiNotes[i][0] = 40
        else:
            continue
        # print(MidiNotes)
...
def Preview(MidiNotes,strings,notePos):
...
def LetsPlay(MidiNotes,strings,notePos):
    # pluckBus = IOPi(0x21)
    # pluckBus.set_port_direction(0, 0x00)   # pins 1-8 are outputs
    # pluckBus.write_port(0, 0x00)   #currently all False

    S0 = Stepper([1,1,2,3])
    S1 = Stepper([1,4,5,6])
    S2 = Stepper([1,7,8,9])
    S3 = Stepper([1,10,11,12])
    S4 = Stepper([1,13,14,15])
    S5 = Stepper([2,9,10,11])

    OpenStrings = [[64,'E4',10],[59,'B3',11],[55,'G3',12],[50,'D2',13],[45,'A2',14],[40,'E2',15]]
    pluck = ar.array('B',(False for v in range(0,len(OpenStrings))))
    for i in range(len(MidiNotes)):
        for string in range(len(strings)):
            for n in range(len(strings[string])):
                if int(strings[string][n][0]) == i:
                    if n>0:
                        ...
                        if strings[string][n][2] == 'StringE4':
                            done = S0.step(difference,direction)
                        elif strings[string][n][2] == 'StringB3':
                            done = S1.step(difference,direction)
                        elif strings[string][n][2] == 'StringG3':
                            done = S2.step(difference,direction)
                        elif strings[string][n][2] == 'StringD2':
                            done = S3.step(difference,direction)
                        elif strings[string][n][2] == 'StringA2':
                            done = S4.step(difference,direction)
                        elif strings[string][n][2] == 'StringE2':
                            ...
                            for x in range(len(OpenStrings)):
                                if MidiNotes[i][0] is OpenStrings[x][0]:
                                    if pluck[x] == False:
                                        ...
                                        continue
                                    print('play ' + OpenStrings[x][1], pluck[x])
                                    pluckBus.write_pin(OpenStrings[x][2], pluck[x])    #pin, pluck
                                    print('time delay: ',MidiNotes[i][1])
                                    time.sleep(MidiNotes[i][1])
                        else:
...
def main():
    ...
    # Preview(MidiNotes,strings, notePos)
