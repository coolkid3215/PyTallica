        ...
        for i in range(len(timeDelay)):
            if sel == 'on':
                if term0 in noteStatus[i]:
                    ...
                    MidiNotes.append([int(temp),float(temp2)])
                    count +=1
                else:
                    continue
            elif sel == 'off':
                if term1 in noteStatus[i]:
                    ...
                    continue
        #if we are gonna treat each string as it's own instance of a song, then they need diff delays
        imGay = 0
        for helpMe in range(len(MidiNotes)):
            if helpMe > 0:
                imGay += float(MidiNotes[helpMe][1])
                MidiNotes[helpMe][1] = imGay
        # print(count)
...
def Open(MidiNotes,notePos,strings):
    ...
    for i in range(len(MidiNotes)):
        if MidiNotes[i][0] <=71 and MidiNotes[i][0] >= 64:   #E4
...
#                                 else:
#                                     continue
#                 elif len(strings[string]) > 0:
#                     for killMe in range(len(strings[string])):
#                         # if strings[string][killMe][3] == 1 and int(strings[string][n][0]) == i-1 and strings[string][n-1][0] != i-1/:
#                             # print(rn)
#                             print('im in',strings[string][killMe],killMe)

def Multiplay(MidiNotes,strings,notePos):
    S0 = guitar([1,1,2,3,0])
    S1 = guitar([1,4,5,6,1])
    S2 = guitar([1,7,8,9,2])
    S3 = guitar([1,10,11,12,3])
    ...
    # i=0
    # for i in range(len(MidiNotes)):
    t0 = threading.Thread(target = S0.multi,args = (strings,0,MidiNotes,notePos))
    t1 = threading.Thread(target = S1.multi,args = (strings,1,MidiNotes,notePos))
    t2 = threading.Thread(target = S2.multi,args = (strings,2,MidiNotes,notePos))
    t3 = threading.Thread(target = S3.multi,args = (strings,3,MidiNotes,notePos))
    t4 = threading.Thread(target = S4.multi,args = (strings,4,MidiNotes,notePos))
    t5 = threading.Thread(target = S5.multi,args = (strings,5,MidiNotes,notePos))
...
def reset(strings,notePos):
    print('we are resetting... i hope')
    OpenStrings = [[0,'E4',1,64],[1,'B3',2,59],[2,'G3',3,55],[3,'D2',4,50],[4,'A2',5,45],[5,'E2',6,40]]
    reset0 = guitar([1,1,2,3,0])
    reset1 = guitar([1,4,5,6,1])
    reset2 = guitar([1,7,8,9,2])
    reset3 = guitar([1,10,11,12,3])
    reset4 = guitar([1,13,14,15,4])
    reset5 = guitar([2,9,10,11,5])
    for string in range(len(strings)):
        if len(strings[string]) >0:
            ...
            difference = int(notePos[str(difference)])
# index, note, string as string, finger flag, string as number, time delay of note
            # if strings[string][n][3]==1:
            if strings[string][n][4] == 0:  #E4
                done = reset0.step(difference,direction)
            elif strings[string][n][4] == 1:   #B3
                done = reset1.step(difference,direction)
            elif strings[string][n][4] == 2:   #G3
                done = reset2.step(difference,direction)
            elif strings[string][n][4] == 3:   #D2
                done = reset3.step(difference,direction)
            elif strings[string][n][4] == 4:   #A2
                done = reset4.step(difference,direction)
            elif strings[string][n][4] == 5:   #E2
                done = reset5.step(difference,direction)
            else:
                continue
            if done:
        ...
        # print('moved stepper ',strings[string][n][4],  )
                ...
                print('should be at zero position')
def main():
    #Start here
    ...
    print('strings\n ', strings)
    for g in range(len(strings)):
        if len(strings[g]) > 0:
            for h in range(len(strings[g])):
                strings[g][h][3] = 0

    # strings = LetsPlay(MidiNotes,strings, notePos)
    strings = Multiplay(MidiNotes,strings,notePos)
    print('strings\n ', strings)
